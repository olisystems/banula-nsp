...
import com.banula.navigationservice.config.ObjectSearchIndexConfig;
 ...
 @Test void indexesAreRepeatableAndUtcBoundsDoNotDependOnJvmTimezone() {
  ...
  try(var client=MongoClients.create(System.getenv("OBJECT_SEARCH_MONGO_URI"))) {
   ...
   try {
    ...
    when(collections.getSmartLocationCollectionName()).thenReturn("collection0");
    ...
    assertEquals(1,service.search("locations",null,null,"DE","OLI",from,from,100,25).total());
