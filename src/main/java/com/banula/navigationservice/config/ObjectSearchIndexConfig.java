...
public class ObjectSearchIndexConfig implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        for (String collection : List.of(
                collections.getSmartLocationCollectionName())) {
            var indexes = mongo.indexOps(collection);
            indexes.ensureIndex(recent(scope()).named("object_search_recent"));
            indexes.ensureIndex(recent(scope().on("countryCode", Sort.Direction.ASC)
                    .on("partyId", Sort.Direction.ASC)).named("object_search_owner_recent"));
            indexes.ensureIndex(recent(scope().on("partyId", Sort.Direction.ASC))
